...
bool Account::withdrawal(int From, int Amount, stringstream &ErrorLog) {
    if (From > FUNDCOUNT - 1) {
        ErrorLog << "Fund " << From << " does not exist." << endl;
        return false;
    } if (Amount <= 0) {
        ErrorLog << "Amount to withdraw from " << ID << From << ", " << endl
    ...
    if (!Funds[From]->withdraw(Amount)) {
        ...
        ErrorLog << "Insufficient funds in " << ID << From << "." << endl;
...
bool Account::deposit(int Into, int Amount, stringstream &ErrorLog) {
    if (Into > FUNDCOUNT - 1) {
        ErrorLog << "Fund " << Into << " does not exist." << endl;
        return false;
    }
    else if (Amount <= 0) {
        ErrorLog << "Amount to deposit from " << ID << Into << ", " << endl
        << "    $" << Amount << ", is negative or zero" << endl;
    }
...
void Account::totalBalOut(ostream &Os) const {
    int Balance = 0;
    for(auto* Fund : Funds){
        Balance += Fund->bal();
    }
    Os << "     Total balance: $" << Balance << endl;
}

bool Account::deposit(int Into, int Amount) {
    stringstream Unused;
    return deposit(Into, Amount, Unused);
}

bool Account::withdrawal(int From, int Amount) {
    stringstream Unused;
    return withdrawal(From, Amount, Unused);
}
